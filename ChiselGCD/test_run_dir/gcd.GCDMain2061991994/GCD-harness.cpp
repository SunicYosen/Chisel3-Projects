...
#endif
#include <iostream>
class GCD_api_t: public sim_api_t<VerilatorDataWrapper*> {
public:
    GCD_api_t(VGCD* _dut) {
        dut = _dut;
        main_time = 0L;
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_loadingValues)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_value2)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_value1)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_outputValid)));
        sim_data.outputs.push_back(new VerilatorSData(&(dut->io_outputGCD)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["GCD.reset"] = 0;
    }
#if VM_TRACE
...
static GCD_api_t * _Top_api;
double sc_time_stamp () { return _Top_api->get_time_stamp(); }
int main(int argc, char **argv, char **env) {
    Verilated::commandArgs(argc, argv);
    VGCD* top = new VGCD;
    std::string vcdfile = "test_run_dir/gcd.GCDMain2061991994/GCD.vcd";
