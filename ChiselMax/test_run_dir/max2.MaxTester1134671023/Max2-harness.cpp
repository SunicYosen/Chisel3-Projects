...
#endif
#include <iostream>
class Max2_api_t: public sim_api_t<VerilatorDataWrapper*> {
public:
    Max2_api_t(VMax2* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_in1)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_in0)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Max2.reset"] = 0;
    }
#if VM_TRACE
...
static Max2_api_t * _Top_api;
double sc_time_stamp () { return _Top_api->get_time_stamp(); }
int main(int argc, char **argv, char **env) {
    Verilated::commandArgs(argc, argv);
    VMax2* top = new VMax2;
    std::string vcdfile = "test_run_dir/max2.MaxTester1134671023/Max2.vcd";
